...
class PasswordResetter
{
    ...
    public function isTokenValid($token, $user, $keySuffix)
    {
        ...
        for ($i = 0; $i <= 24; $i++) {
            if ($this->doesPasswordResetTokenMatch($token, $user, $keySuffix, $now + $i * 60 * 60)) {
                return true;
            }
        }
    ...
    /***
     * Ensure that a password reset token is valid.
     *
     * @param string $token The current hashed token to match against
     * @param array $user The user information.
     * @param string $keySuffix The suffix used in generating a token.
     * @param int $expiryTimestamp The expiration timestamp to use or null to generate one from
     *                                  the current timestamp.
     */
    private function doesPasswordResetTokenMatch($token, $user, $keySuffix, $expiryTimestamp)
    {
        $expiry = strftime('%Y%m%d%H', $expiryTimestamp);

        return $this->secureHashMatchesData(
    ...
    /**
     * Generates a unique hash using the configured password hash algorithm.
     * The hash identifier is a string that differentiates the hash in some way.
     *
     * Use secureHashMatchesData() method to check that a generated hash matches the original data.
     *
    ...
    /***
     * Verifies that a hash generated by the generateSecureHash() method is valid.
     *
     * @param string $hashIdentifier A unique string that identifies the hash in some way, can,
     *                               for example, be user information or can contain an expiration date,
     *                               or whatever.
     * @param string $data Any data that needs to be hashed securely, ie, a password.
     * @param string $token The received hash to be verified.
     * @return bool
     */
    protected function secureHashMatchesData($hashIdentifier, $data, $token)
    {
        $stringToHash = $hashIdentifier
            . $this->getSalt()
            . $data;

        return $this->passwordHelper->verify($stringToHash, $token);
    }
     ...
     * @return string
     * @deprecated This method should not be used for password resetting as it does not provide a secure hash.
     */
